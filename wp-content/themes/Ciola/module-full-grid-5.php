 <?php /* Module: Grid - 5 Articles */ 
	...
	echo '<div class="cb-grid-5 '.$cb_no_byline.'clearfix">';
    
	if (is_category()) { 
			
			$current_cat_id = get_query_var('cat');
	
			$cb_featured_qry = array( 'post_type' => 'post', 'meta_key' => 'cb_featured_post', 'cat' => $current_cat_id, 'posts_per_page' => '5', 'orderby' => 'date', 'order' => 'DESC',  'post_status' => 'publish', 'meta_value' => 1,  'meta_compare' => '==', 'ignore_sticky_posts' => true );
			$qry = new WP_Query( $cb_featured_qry );
		   
			if ($qry->post_count == 0) { 
				$qry = NULL;
				$qry = new WP_Query(array( 'posts_per_page' => '5', 'no_found_rows' => true, 'post_type' => 'post', 'cat' => $current_cat_id, 'post_status' => 'publish', 'ignore_sticky_posts' => true )  );
			}
	
    } else { 
			 $cb_featured_qry = array( 'post_type' => 'post', 'meta_key' => 'cb_featured_post', 'posts_per_page' => '5', 'orderby' => 'date', 'cat' => $cb_filtered_cat, 'order' => 'DESC',  'post_status' => 'publish', 'meta_value' => 1,  'meta_compare' => '==', 'ignore_sticky_posts' => true );
			$qry = new WP_Query( $cb_featured_qry );
			   
			if ($qry->post_count == 0) { 
					$qry = NULL;
					$qry = new WP_Query(array( 'posts_per_page' => '5', 'no_found_rows' => true, 'post_type' => 'post',  'cat' => $cb_filtered_cat, 'post_status' => 'publish', 'ignore_sticky_posts' => true )  );
			}
	}
